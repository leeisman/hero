...
// The UserActiveRecordQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type UserActiveRecordQueryRuleFunc func(context.Context, *ent.UserActiveRecordQuery) error

// EvalQuery return f(ctx, q).
func (f UserActiveRecordQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.UserActiveRecordQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.UserActiveRecordQuery", q)
}

// The UserActiveRecordMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type UserActiveRecordMutationRuleFunc func(context.Context, *ent.UserActiveRecordMutation) error

// EvalMutation calls f(ctx, m).
func (f UserActiveRecordMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.UserActiveRecordMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.UserActiveRecordMutation", m)
}
